...
4/8
- Ended up just deleting the whole repo and making a new one was easier so we took that method to fix our problem. Working on pulling data from the API and cleaning up the code we already had. 
4/13
...
Wait about 30 seconds for Metabase to finish starting, then open [http://localhost:3000](http://localhost:3000) to confirm it loads.

# Docker Shortcuts

> To stop the stack when you're done: `docker compose down`
...
| Part | What you do |
|------|-------------|
| **Part 1** | Load all of our excels; fetch weather from the Open-Meteo API |
...
Metabase is already running at [http://localhost:3000](http://localhost:3000).

**First-time setup (one time only):**
1. Complete the Metabase welcome wizard
2. When asked to add a database, choose **PostgreSQL** and enter:
...
1. Click **+ New → Dashboard**, give it a name
2. Add questions (charts) using **+ New → Question**
3. Choose the `postgres` database and pick one of your analytics tables
4. Select a visualization type and justify your choice in your submission
